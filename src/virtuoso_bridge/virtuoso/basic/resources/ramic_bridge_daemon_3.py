...
def _mark_request_timed_out(request_id, request_generation):
    """Atomically mark only the still-active generation as timed out."""
    with _STATE_LOCK:
        entry = _REQUESTS.get(request_id)
        if (
            not entry
...
def _record_duplicate(request_id, replayed):
    """Record duplicate metadata without replacing the terminal state."""
    with _STATE_LOCK:
        entry = dict(_REQUESTS.get(request_id) or {"request_id": request_id})
    ...
    while True:
        ...
        try:
            ...
            if len(result) < max_response_bytes:
                result.extend(ch[: max_response_bytes - len(result)])
            else:
                overflow = True
...
def _admit_external_connection(conn, addr):
    ...
    try:
        ...
        _safe_sendall(conn, ("\x15%s" % exc).encode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        _safe_sendall(conn, ("\x15JSONDecodeError: %s" % exc).encode("utf-8"))
...
def handle_external_connection(admitted):
    ...
    try:
        ...
        response_digest = (
            ...
            else hashlib.sha256(bytes(returnData)).hexdigest()
        )
        marker = "STX" if returnData[:1] == b"\x02" else "NAK"
        payload_bytes = bytes(returnData[1:])
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        try:
            ...
            if e.errno == errno.EADDRINUSE:
                ...
                sys.exit(1)
            raise
        # CIW execution remains deliberately serial.  A small backlog lets
        # healthy clients queue while a half-open sender is bounded by the
        # per-connection request-read deadline above.
        s.listen(8)
        s.settimeout(1.0)
