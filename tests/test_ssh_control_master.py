...
def test_exclusive_text_upload_does_not_chmod_existing_parent(monkeypatch) -> None:
    ...
    encoded = shlex.split(remote_command)[2].split("printf %s ", 1)[1].split(" | base64", 1)[0]
    remote_command = base64.b64decode(encoded).decode("utf-8")
    assert remote_command.startswith("(\nset -eu\numask 077\n")
    assert 'ln -T -- "$payload" /home/designer/.cdsinit.tmp' in remote_command
    assert "chmod 755" not in remote_command
    assert "mkdir -p" not in remote_command


def test_exclusive_persistent_text_upload_scopes_shell_state(monkeypatch) -> None:
    monkeypatch.setattr("virtuoso_bridge.transport.ssh.load_vb_env", lambda: None)
    monkeypatch.setattr("virtuoso_bridge.transport.ssh._setup_command_log", lambda: None)
    commands: list[str] = []
