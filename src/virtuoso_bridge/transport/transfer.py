...
def _text_upload_script(plan: TextUploadPlan, writer: str) -> str:
    ...
    lines = [
        ...
        "trap 'exit 130' INT",
        "trap 'exit 143' TERM",
        f"mkdir -p {remote_dir_q}" if plan.create_parent else f"test -d {remote_dir_q}",
        'mkdir -- "$work"',
        "created=1",
        writer,
        ...
        "fi",
        (f'ln -T -- "$payload" {remote_path_q} && rm -- "$payload"'
         if plan.exclusive_create else f'mv -f "$payload" {remote_path_q}'),
        'rmdir -- "$work"',
        "created=0",
        "trap - EXIT HUP INT TERM",
        ")",
...
def build_text_upload_plan(remote_path: str, payload: bytes, *, create_parent: bool = True, exclusive_create: bool = False) -> TextUploadPlan:
    ...
    return TextUploadPlan(
        ...
        exclusive_create=plan.exclusive_create,
    )
