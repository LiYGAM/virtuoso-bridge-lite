...
class SSHRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self._use_control_master = (
            ...
            and ((_force_cm or not _disable_cm) if control_master is None else bool(control_master))
        )
    ...
    def upload(
        ...
        *,
        ...
        result = self._execute_tar_upload_plans(plans, budget, retry_transport_errors=retry_transport_errors)
    ...
    def upload_batch(
        ...
        *,
        ...
        return self._execute_tar_upload_plans(plans, budget, retry_transport_errors=retry_transport_errors)
    ...
    def upload_text(
        ...
        *,
        ...
        plan = build_text_upload_plan(remote_path, text_bytes, create_parent=create_parent, exclusive_create=exclusive_create)
        if self._paramiko_backend is not None:
        ...
        *,
        ...
        for plan in plans:
            if self._paramiko_backend is not None:
                ...
                result = self._execute_openssh_upload_plan(plan, budget, retry_transport_errors=retry_transport_errors)
            if result.returncode != 0:
                return result
        return CommandResult(returncode=0, stdout="", stderr="")
    ...
    def _common_ssh_options(
        self, *, control_master: bool = True, include_control_master: bool | None = None
    ) -> list[str]:
        """SSH options shared by both ssh and scp commands."""
        if include_control_master is not None:
            control_master = include_control_master
        opts: list[str] = [
            "-o", "BatchMode=yes",
